...
export default function CustomSelectorManager({
  ...
  const editSelector = (e, idx) => {
    clearTimeout(debounceTimeout.current);

    debounceTimeout.current = setTimeout(() => {
      selectors.map((i) => removeSelector(i))
      selectors[idx] = {name: e.target.innerText, label: e.target.innerText};
    ...
    <div className="gjs-custom-selector-manager p-2 flex flex-col gap-2 text-left">
      ...
      >
        ...
        {selectors.map((selector, idx) => (
          ...
          >
            <div contentEditable={true} onInput={(e) => editSelector(e, idx)}>{selector.getLabel()}</div>
